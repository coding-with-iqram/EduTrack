// import React from "react";

// const ThankYouOverlay = ({ onClose }) => {
//   return (
//     <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/30">
//       <div className="relative bg-white p-6 sm:p-8 rounded-2xl shadow-lg w-full max-w-md sm:max-w-lg text-center">
//         {/* Close (X) Button */}
//         <button
//           onClick={onClose}
//           className="absolute top-3 right-3 text-gray-500 hover:text-gray-800 text-xl"
//         >
//           &times;
//         </button>

//         <h1 className="text-2xl sm:text-3xl font-semibold text-indigo-700 mb-4">🎉 Thank You!</h1>
//         <p className="text-gray-700 text-base sm:text-lg">
//           Hello User. Welcome to <strong>EduTrack</strong>.<br />
//           <span className="block mt-2">
//             Progress, not perfection. Stay consistent, and success will follow.
//           </span>
//         </p>
//       </div>
//     </div>
//   );
// };

// export default ThankYouOverlay;
